...
int main(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (a == "--ip")
      ...
      command_s = std::stod(val());
    else if (a == "--observe-s")
      observe_s = std::stod(val());
    else if (a == "--cmd-rate")
  ...
  if (std::abs(tau_ff) > 0.5 * limit) {
    std::cerr << "--tau " << tau_ff << " N*m is more than half j" << joint << "'s " << limit
              << " N*m clamp. This is a validation harness, "
                 "not a motion tool — keep the feedforward small.\n";
    return 2;
  }

  std::printf(
      "[jtau] joint=j%d tau_ff=%+.3f N*m (clamp %.1f) command=%.1fs "
      "observe=%.1fs cmd_rate=%.0fHz rate=%.0fHz sim=%s dry_run=%s\n",
      joint, tau_ff, limit, command_s, observe_s, cmd_rate_hz, rate_hz, use_sim ? "yes" : "no",
      dry_run ? "yes" : "no");
  std::printf(
      "[jtau] watchdog: cmd_timeout=%.3fs decay=%.3fs — after the "
  ...
  if (dry_run) {
    ...
    for (int i = 0; i < kNumJoints; ++i)
      std::printf("  j%d  q=%+8.4f rad  tau_meas=%+8.3f  gravity=%+8.3f  clamp=%.1f N*m\n", i,
                  fb.q[i], fb.tau[i], g[i], p.torque_limit[i]);
    std::printf(
  ...
  std::printf(
      "[jtau] the arm is COMPLIANT here — it will yield if you push it, "
  ...
  if (use_sim)
    std::printf(
        "[jtau] (--sim is a static echo: measured tau never responds and "
