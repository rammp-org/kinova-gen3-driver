...
Arbiter::Arbiter(CommandSink& downstream, StreamSink& downstream_stream,
    ...
    : down_(downstream),
      down_stream_(downstream_stream),
      down_grip_(downstream_gripper),
      mode_(mode),
      rng_(seed ? seed : std::random_device{}()) {}

Token Arbiter::mint() {
  Token t{};
  const uint64_t a = rng_(), b = rng_();
  std::memcpy(t.data(), &a, sizeof(a));
  std::memcpy(t.data() + 8, &b, sizeof(b));
  return t;
}
...
GrantResult Arbiter::grant(const std::string& owner_id) {
  bool need_halt = false;
  {
    ...
    if (estopped_.load(std::memory_order_acquire))
      return {false, Token{}, generation_, "e-stopped"};
...
void Arbiter::estop_clear() {
  ...
  estopped_.store(false,
                  std::memory_order_release);  // exits to no-owner, never straight back to owned
}

ArbitrationStatus Arbiter::status() const {
  std::lock_guard<std::mutex> l(m_);
  return {mode_,    estopped_.load(std::memory_order_acquire), owned_, owner_id_, generation_,
          rejected_};
}
...
GoalResponse Arbiter::on_trajectory_goal(const TrajectoryGoal& g) {
  ...
  if (!admit(g.token)) {
    ++rejected_;
    return GoalResponse::kRejectUnauthorized;
  }
...
StreamOpenResult Arbiter::on_stream_open(const StreamOpenRequest& r) {
  ...
  if (!admit(r.token)) {
    ++rejected_;
    return {false, result_code::kNotAuthorized, "not authorized"};
  }
