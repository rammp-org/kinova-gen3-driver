...
**Deliberately out of scope, per the spec's decomposition:** `GripperSink`, `Supervisor` and `Arbiter` wiring, the `Grasp` action, and stall *detection* (as opposed to stall *simulation*) are all stage 2. The `rt_safety_test` case the spec asks for covers `GripperController` in a live chain — it belongs with stage 2's Supervisor wiring, where there is a mode running to put it behind; Task 4's decorator tests cover the stamp itself.
