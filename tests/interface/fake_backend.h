...
class FakeBackend : public StreamPort, public ActionServerPort {
 public:
  ...
  ArmState last_state() const {
    std::lock_guard<std::mutex> l(m_);
    return states_.back();
  }
  TrajectoryResult last_result() const {
    std::lock_guard<std::mutex> l(m_);
    return results_.back().second;
  }
  GoalId last_result_id() const {
    std::lock_guard<std::mutex> l(m_);
    return results_.back().first;
  }
