...
TrajectoryGoal goal_with(const Token& t) {
  TrajectoryGoal g;
  g.token = t;
  return g;
}
...
TEST(Arbiter, AnEstopClearInsideTheEstopWindowCannotUnlatchTheStop) {
  ...
  e.join();
  EXPECT_TRUE(arb.status().estopped)
      << "an estop_clear un-latched a stop that had already been ordered";
...
TEST(Arbiter, EstopClearExitsToNoOwnerNotToOwned) {
  ...
  EXPECT_FALSE(arb.status().owned);
  EXPECT_EQ(arb.on_trajectory_goal(goal_with(t)),
            GoalResponse::kRejectUnauthorized);  // old token dead
  const GrantResult g = arb.grant("planner");
...
TEST(Arbiter, StatusReflectsOwnerAndRejectionCount) {
  ...
  CancelRequest c;
  arb.on_trajectory_cancel(c);  // reject 2
