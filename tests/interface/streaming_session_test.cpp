...
TEST(StreamingSession, ZeroOrNegativeTimeoutIsRefused) {
  StreamingSession s;
  EXPECT_FALSE(
      s.open(req(SetpointKind::kJointPosition, ControlModeKind::kPosition, 0.0), 0.0).accepted);
  EXPECT_FALSE(
      s.open(req(SetpointKind::kJointPosition, ControlModeKind::kPosition, -1.0), 0.0).accepted);
...
TEST(StreamingSession, SecondOpenIsRefusedCloseFirst) {
  ...
  EXPECT_FALSE(
      s.open(req(SetpointKind::kJointPosition, ControlModeKind::kImpedance), 0.0).accepted);
...
TEST(StreamingSession, ExpiresOnlyAfterTheDeadlineAndFreshSetpointsPushItOut) {
  StreamingSession s;
  ASSERT_TRUE(
      s.open(req(SetpointKind::kJointPosition, ControlModeKind::kPosition, 0.1), 0.0).accepted);
...
TEST(PairSupported, VelocityModeRefusesEveryNonVelocityKind) {
  for (auto k : {SetpointKind::kJointPosition, SetpointKind::kEePose, SetpointKind::kJointTorque}) {
