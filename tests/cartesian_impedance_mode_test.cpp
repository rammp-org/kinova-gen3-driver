...
TEST(CartesianImpedance, MatchesIndependentlyComputedLaw) {
  ...
  m.on_enter(fb);
  Pose tgt = dyn.fk(fb.q);
  tgt.p.x() += 0.05;
  tgt.p.z() -= 0.03;  // displace
  m.set_target(tgt);
  JointCommand c;
  m.compute(fb, 0.001, c);
...
TEST(CartesianImpedance, TorqueIsClamped) {
  ...
  m.on_enter(fb);
  Pose tgt = dyn.fk(fb.q);
  tgt.p += Eigen::Vector3d(0.3, 0.3, 0.3);  // big error
  m.set_target(tgt);
  JointCommand c;
  m.compute(fb, 0.001, c);
...
TEST(CartesianImpedanceNullspace, PostureTorqueProducesNoTaskWrench) {
  ...
  enter.q = sample_q();
  enter.qd.setZero();
  m_off.on_enter(enter);
  m_on.on_enter(enter);  // q_rest := sample_q()

  JointFeedback fb;
  fb.q = sample_q();
  fb.q[2] += 0.15;  // posture error, off target
  fb.qd.setZero();
...
TEST(CartesianImpedanceNullspace, FixedRestPostureActiveAndInNullspace) {
  ...
  Dynamics dyn(URDF_PATH);
  JointVec rest = sample_q();
  rest[2] += 0.15;
  rest[5] -= 0.12;  // fixed rest != entry
  ...
  CartesianImpedanceParams on = off;
  on.nullspace_on = true;
  on.nullspace_use_fixed_rest = true;
  on.nullspace_q_rest = rest;
  on.nullspace_kp = 8.0;
  on.nullspace_kd = 0.0;
  CartesianImpedanceMode m_on(dyn, on);
  ...
  JointFeedback fb;
  fb.q = sample_q();
  fb.qd.setZero();  // enter & compute at entry
  m_off.on_enter(fb);
  m_on.on_enter(fb);  // q_rest := rest (on), q (off)
  m_off.set_target(dyn.fk(fb.q));
  m_on.set_target(dyn.fk(fb.q));  // zero task error
...
TEST(CartesianImpedanceNullspace, ContinuousJointPostureTakesShortWayAroundTheWrap) {
  ...
  m_off.on_enter(fb);
  m_on.set_target(dyn.fk(fb.q));
  m_off.set_target(dyn.fk(fb.q));  // zero task error
  ...
  Eigen::Matrix<double, kNumJoints, kNumJoints> N =
      Eigen::Matrix<double, kNumJoints, kNumJoints>::Identity() - J.transpose() * A.ldlt().solve(J);
