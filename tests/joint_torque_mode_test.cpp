...
TEST(JointTorque, WatchdogZerosStaleFeedforward) {
  Dynamics dyn(URDF_PATH);
  JointTorqueMode m(dyn,
                    {1.0, 0.0, JointVec::Constant(1e9), 0.05, 0.0});  // timeout 50ms, hard zero
  JointFeedback fb;
  fb.q.setZero();
  fb.q[1] = M_PI / 2;
  fb.qd.setZero();
...
TEST(JointTorque, WatchdogDecayRampIsMonotoneToZero) {
  ...
  for (int k = 0; k < 55; ++k) {
    ...
    EXPECT_GE(cur_pos, -1e-9) << "positive-target applied ff went negative at decay cycle " << k;
    ...
    EXPECT_LE(cur_neg, 1e-9) << "negative-target applied ff went positive at decay cycle " << k;
    prev_pos = cur_pos;
    prev_neg = cur_neg;
  }
